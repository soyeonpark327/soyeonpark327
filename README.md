...
- Dashboard에 자리만 잡혀있던 "OGQ 출시 검토" 기능 설계·구현 — 완료된 프로젝트의 OCR 캡션을 키워드로 뽑아 OGQ 마켓에 비슷한 스티커가 이미 있는지 검색해 참고 목록과 출시 체크리스트를 보여줌. 실제 회원가입부터 업로드·OCR·번역 파이프라인 완료, 실제 OGQ 검색까지 Playwright로 전 과정 검증하다가, 이번 기능과는 무관한 기존 버그(에디터 다운로드 시 stale regionId로 완료 처리가 실패하는 문제)를 별도로 찾아내 보고 ([#68](https://github.com/Linkshimcat/Glocalizer/pull/68))
- 프로덕션에서 OGQ 연동 기능 전체가 실패하는 문제 진단·수정 — 배포된 Render 백엔드에 OGQ_API_KEY 환경변수가 아예 등록돼 있지 않았던 게 원인임을 render.yaml과 실제 프로덕션 API 응답을 대조해 확인하고, 다른 시크릿과 같은 패턴으로 슬롯을 추가. 환경변수 등록 후 실제 프로덕션 엔드포인트를 다시 호출해 정상 동작 확인 ([#69](https://github.com/Linkshimcat/Glocalizer/pull/69))
- 이전 PR에서 실수로 유실된 커밋 발견·복구 — 샘플 선택 모달 UI 버튼 배치 커밋을 PR이 머지된 뒤에야 브랜치에 푸시해버려 main에 한 번도 반영되지 않았던 걸 git 히스토리 대조로 발견하고 cherry-pick으로 복구. 같은 PR에서 샘플 선택 모달이 12개(3x4) 고정으로만 보이던 것도 24개+스크롤로 확장 ([#70](https://github.com/Linkshimcat/Glocalizer/pull/70))
- 브러시 지우기 도구 후속 개선 2건 — (1) 칠한 자리에 늘 반투명 빨강만 덧그려 실제 지우기 결과(투명/단색)를 다운로드해야만 알 수 있던 것을, 사각형 도구처럼 실제 모드를 그대로 실시간으로 보여주도록 수정 (2) PR #61에서 브러시 스트로크가 텍스트에 가로막히는 걸 막으려 모든 텍스트에 걸었던 pointer-events-none의 부작용으로 브러시 모드에서 텍스트를 아예 선택·이동·크기조절할 수 없었던 원인을 추적해, 선택된 텍스트만 예외로 살려두고 브러시가 선택을 강제 해제하던 로직도 제거. 실제 파이프라인 완료 후 캔버스 픽셀 값과 실제 다운로드 결과물까지 대조해 검증 ([#71](https://github.com/Linkshimcat/Glocalizer/pull/71))
- 번역 시작 버튼이 업로드·언어 선택 후 몇 초간 잠겨 있던 문제 진단·수정 — 원인이 백그라운드 자동저장 상태(cloudSaving)에 버튼을 묶어둔 것임을 찾고, 시작 함수가 이미 저장을 직접 큐에 이어 붙인다는 걸 확인해 조건을 제거. 업로드 API를 4초 지연시켜 저장 중에도 버튼이 즉시 활성화되고 프로젝트가 중복 생성되지 않는 것을 Playwright로 검증 ([#80](https://github.com/Linkshimcat/Glocalizer/pull/80))
- 클린업 후 글자 아래끝 자투리가 남는 문제 근본 원인 규명·수정 — 프로덕션 DB에서 사용자가 올린 실제 원본을 내려받아 파이프라인 코드로 재현하고, 잔상이 전부 OCR 박스 아래 6px 구간(314~319행)에 몰려 있음을 행 단위로 확인해 마스크 스캔 범위가 박스 크기와 무관하게 3px로 고정된 게 원인임을 특정. 이어진 획만 제한 거리까지 따라가는 성장 로직으로 떨어진 캐릭터는 보존하면서 잔상 213px → 2px로 제거, 수정 전 코드에서 실패하는 회귀 테스트 추가 ([#82](https://github.com/Linkshimcat/Glocalizer/pull/82))
- 클린업이 같은 이미지에서 들쭉날쭉하고 단순한 배경을 복잡하다고 판정하는 문제 근본 원인 3가지 규명·수정 — 프로덕션 실제 이미지와 OGQ 스티커에 OCR 박스 흔들림 12종을 적용해 기존 코드와 나란히 비교하는 하니스를 만들어 (1) 링 전체 평균 기반 배경 판정이 캐릭터 몇 픽셀에 뒤집히는 것(투명 스티커 통과율 4/12), (2) Luna/Paddle 글자 판독 불일치 플래그가 배경과 무관하게 자동 정리를 통째로 막던 것, (3) 말풍선 테두리를 참조해 채움색이 회색 얼룩이 되는 것을 각각 실측으로 확인. 변 단위 배경 판정 + 박스 흔들림 다수결로 통과율 12/12로 안정화하고, 합성 복잡 배경으로 오판 위험을 스윕해 임계값(0.85)을 근거 있게 선정, 효과 없던 시도(색 대비 마스크)는 되돌리고 한계로 문서화. 실제 Luna OCR 파이프라인 반복 실행으로 종단 검증 ([#84](https://github.com/Linkshimcat/Glocalizer/pull/84))
